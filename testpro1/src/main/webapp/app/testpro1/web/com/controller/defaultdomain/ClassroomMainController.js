Ext.define('Testpro1.testpro1.web.com.controller.defaultdomain.ClassroomMainController', {
     extend: 'Testpro1.view.fw.frameworkController.FrameworkViewController',
     alias: 'controller.ClassroomMainController',
     ...
     onTreeRefreshClick: function(event, toolEl, panelHeader) {
          ...
          var tree = this.view.down('#ClassroomTree');
     ...
     refreshMainForm: function(but, data) {
          ...
          if (data != null && data instanceof Object) {
               var grid = this.view.down('#ClassroomGrid');
               grid.store.add(data);
               var tree = this.view.down('#ClassroomTree');
               var rootNode = tree.getRootNode();
               var childNode = {
     ...
     hideDataBrowser: function() {
          var form = this.view.down('#Classroom');
          var grid = this.view.down('#ClassroomGrid');
          var tree = this.view.down('#ClassroomTreeContainer');
          grid.setHidden(true);
     ...
     treeClick: function(me, record, item, index, e, eOpts) {
          if (record.data.leaf) {
               var gridPanel = this.view.up().down('#ClassroomGrid');
               ...
               var formPanel = this.getView().up().down("#Classroom");
     ...
     onGridItemClick: function(me, record, item, index, e, eOpts) { /** Finding record by id */
          ...
          var treePanel = this.view.up().up().down('#ClassroomTree');
     ...
     renderFormValue: function(val, me) {
          store = this.view.up().down('#Classroom').down('#' + me.column.dataIndex + '').store;
     ...
     setGridData: function(data) {
          var gridStore = this.view.down('#ClassroomGrid').store;
          gridStore.removeAll();
     ...
     onDeleteActionColumnClickMainGrid: function(grid, rowIndex) {
          var recordId = grid.store.data.items[rowIndex].data.classId;
          me = this;
     ...
     loadData: function(responseData) {
          this.view.down('#Classroom').getViewModel().setData(responseData.response.data);
     },
